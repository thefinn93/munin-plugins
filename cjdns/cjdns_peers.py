...
if config:
    ...
    for peer in allPeers:
        print "%s.label %s" % (allPeers[peer]['publicKey'][0:10], peer)
        print "%s.draw AREASTACK" % allPeers[peer]['publicKey'][0:10]
    print "total.label total"
    print "total.type GAUGE"
    print "total.draw LINE2"
else:
    for peer in allPeers:
        print "%s.value %s" % (allPeers[peer]['publicKey'][0:10], allPeers[peer])
